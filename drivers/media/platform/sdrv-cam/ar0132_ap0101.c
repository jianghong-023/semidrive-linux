...
static int ap0101_read_reg8(struct ar0132_dev *sensor, u16 reg, u8 *val)
{
...
static int ap0101_write_reg8(struct ar0132_dev *sensor, u16 reg, u8 val)
{
	struct i2c_client *client = sensor->i2c_client;
	struct i2c_msg msg;
	u8 buf[3];
...
static int ap0101_read_reg32(struct ar0132_dev *sensor, u16 reg, u32 *val)
{
	struct i2c_client *client = sensor->i2c_client;
	struct i2c_msg msg[2];
	u8 buf[4];
...
static int ap0101_write_reg32(struct ar0132_dev *sensor, u16 reg, u32 val)
{
	struct i2c_client *client = sensor->i2c_client;
	struct i2c_msg msg;
	u8 buf[6];
...
static struct v4l2_subdev *ar0132_get_interface_subdev(struct v4l2_subdev *sd)
{
...
static int ar0132_s_power(struct v4l2_subdev *sd, int on)
{
	...
	struct v4l2_subdev *interface_sd = ar0132_get_interface_subdev(sd);
...
static int ar0132_initialization(struct ar0132_dev *sensor)
{
	...
	val16 = 0;
	ap0101_read_reg16(sensor, reg, &val16);
	dev_err(&client->dev, "1-reg=0x%x, val16=0x%x\n", reg, val16);
	val16 = 0x405;
	ap0101_write_reg16(sensor, reg, val16);
	usleep_range(10000, 11000);
	val16 = 0;
	ap0101_read_reg16(sensor, reg, &val16);
	dev_err(&client->dev, "2-reg=0x%x, val16=0x%x\n", reg, val16);

	ap0101_change_config(sensor);
#endif
