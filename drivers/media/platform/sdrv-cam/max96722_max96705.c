...
static int max96722_read_reg(struct max96722_dev *sensor, u16 reg, u8 * val)
{
	struct i2c_client *client = sensor->i2c_client;
...
static int max96705_write_reg(struct max96722_dev *sensor, int channel, u8 reg,
			      u8 val)
{
	struct i2c_client *client = sensor->i2c_client;
	struct i2c_msg msg;
...
static int max96705_read_reg(struct max96722_dev *sensor, int channel, u8 reg,
			     u8 * val)
{
	struct i2c_client *client = sensor->i2c_client;
	struct i2c_msg msg[2];
...
static int max96705_access_sync(struct max96722_dev *sensor,  int channel, int write, u16 reg, u8* val_r, u8 val_w)
{
	int ret = -1;

	if(channel < 0 || channel > 3) {
		dev_err(&sensor->i2c_client->dev, "%s: chip max96705 ch=%d, invalid channel.\n", __FUNCTION__, channel);
		return ret;
	}
...
static struct v4l2_subdev *max96722_get_interface_subdev(struct v4l2_subdev *sd)
{
...
static int max96705_check_chip_id(struct max96722_dev *sensor)
{
	...
	for(i = 0; i < 4; i++) {
		ret = max96705_read_reg(sensor, i, 0x1E, &chip_id);
		if(ret < 0) {
			dev_err(&client->dev,
...
static int max96722_initialization(struct max96722_dev *sensor)
{
	...
	for(i = 0; i < reglen; i++)
	{
		max96722_write_reg(sensor, max96722_reg[i].nRegAddr, max96722_reg[i].nRegValue);
	...
	for (i = 0; i < MAX_CAMERA_NUM; i++) {
		...
		msleep(10);
		max96705_write_reg(sensor, (MAX96705_DEF-MAX96705_CH_A)>>1, 0x00, MAX96705_CH_A + i * 2);
		msleep(10);
