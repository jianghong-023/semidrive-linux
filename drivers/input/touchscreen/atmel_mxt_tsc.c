...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	snprintf(data->phys, sizeof(data->phys), "input/ts_main");
	...
	error = of_property_read_u32(client->dev.of_node, "addr_ds941", &data->addr_ds941);
	if (error < 0) {
		dev_err(&client->dev, "Missing addr_ds941\n");
		return error;
	}

	error = of_property_read_u32(client->dev.of_node, "addr_ds948", &data->addr_ds948);
	if (error < 0) {
		dev_err(&client->dev, "Missing addr_ds948\n");
		return error;
	}

	error = of_property_read_u32(client->dev.of_node, "reset_channel", &data->reset_channel);
	if (error < 0) {
		dev_err(&client->dev, "Missing reset_channel\n");
		return error;
	}

	dev_err(&client->dev, "addr_ds941=%#x, addr_ds948=%#x, reset_channel=%d\n",
		data->addr_ds941, data->addr_ds948, data->reset_channel);

	error = du90ub941_or_947_enable_i2c_passthrough(client, data->addr_ds941);
	if (error)
		return error;

	error = du90ub941_or_947_enable_int(client, data->addr_ds941);
	if (error)
		return error;
