...
#if 1
	rc = of_property_read_u32(core_node, "addr_ds941", &pdata->addr_ds941);
	if (rc < 0) {
		pr_err("%s, Missing addr_ds941\n", __func__);
		return rc;
	}

	rc = of_property_read_u32(core_node, "addr_ds948", &pdata->addr_ds948);
	if (rc < 0) {
		pr_err("%s, Missing addr_ds948\n", __func__);
		return rc;
	}

	rc = of_property_read_u32(core_node, "reset_channel", &pdata->reset_channel);
	if (rc < 0) {
		pr_err("%s, Missing reset_channel\n", __func__);
		return rc;
	}

	pr_err("%s, addr_ds941=%#x, addr_ds948=%#x, reset_channel=%d\n", __func__,
		pdata->addr_ds941,pdata->addr_ds948, pdata->reset_channel);

	pdata->irq_gpio = of_get_named_gpio(core_node, "cy,irq_gpio", 0);
	if(!gpio_is_valid(pdata->irq_gpio)) {
		printk(KERN_ERR "cyttsp6 irq_gpio(%d) is invalid\n", pdata->irq_gpio);
		goto fail_free;
	}
#else
	/* Required fields */
