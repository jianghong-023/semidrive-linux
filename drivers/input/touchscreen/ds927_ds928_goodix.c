...
static int du90ub927_i2c_read(struct goodix_ts_data *ts,
                              u8 reg, u8 *buf, int len)
{
    ...
    msgs[0].flags = 0;
    msgs[0].addr  = ts->addr_ds927;
    msgs[0].len   = 1;
    msgs[0].buf   = &reg;

    msgs[1].flags = I2C_M_RD;
    msgs[1].addr  = ts->addr_ds927;
    msgs[1].len   = len;
    msgs[1].buf   = buf;
...
static int du90ub927_i2c_write(struct goodix_ts_data *ts,
                               u8 reg, u8 *buf, int len)
{
    u8 *addr_buf;
    struct i2c_msg msg;
...
static int du90ub928_i2c_read(struct goodix_ts_data *ts,
                              u8 reg, u8 *buf, int len)
{
    struct i2c_msg msgs[2];
    int ret;
    ...
    msgs[0].flags = 0;
    msgs[0].addr  = ts->addr_ds928;
    msgs[0].len   = 1;
    msgs[0].buf   = &reg;

    msgs[1].flags = I2C_M_RD;
    msgs[1].addr  = ts->addr_ds928;
    msgs[1].len   = len;
    msgs[1].buf   = buf;
...
static int du90ub928_i2c_write(struct goodix_ts_data *ts, u8 reg,
                               const u8 *buf,
                               unsigned len)
{
    u8 *addr_buf;
...
static int du90ub927_928_gpio2_output(struct goodix_ts_data *ts)
{
    u8 dreg, dval;
...
static int du90ub927_928_gpio1_output(struct goodix_ts_data *ts)
{
    u8 dreg, dval;
...
static int du90ub927_928_gpio1_input(struct goodix_ts_data *ts)
{
    u8 dreg, dval;
                           ...
                           const struct i2c_device_id *id)
{
    ...
    val = 0;
    error = of_property_read_u32(client->dev.of_node, "addr_ds927", &val);

    if (error < 0) {
        dev_err(&client->dev, "Missing addr_ds927\n");
    }

    ts->addr_ds927 = (u8)val;
    val = 0;
    error = of_property_read_u32(client->dev.of_node, "addr_ds928", &val);

    if (error < 0) {
        dev_err(&client->dev, "Missing addr_ds928\n");
    }
    ...
    dval = 0;
    du90ub927_i2c_read(ts, dreg, &dval, 1);
    dev_err(&client->dev, "927 reg=0x%x, val=0x%x\n", dreg, dval);
    dreg = 0x8;
    dval = 0;
    du90ub927_i2c_read(ts, dreg, &dval, 1);
    dev_err(&client->dev, "927 reg=0x%x, val=0x%x\n", dreg, dval);
...
static struct i2c_driver goodix_ts_driver = {
    ...
    .driver = {
        .name = "Semidrive DS927 DS928 Goodix-TS",
