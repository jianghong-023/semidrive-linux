...
#define REG_CE_TRNG_CONTROL   (CRYPTOENGINE_APB_AB0_BASE_ADDR + (0x8100<<0))
...
#define TRNG_STATUS_OFFSET (0x8130<<0)
...
#define CE0_TRNG_STATUS_OFFSET (0x100<<0)
#define CE0_TRNG_NUM0_OFFSET (0x104<<0)
...
{
	return readl(ce_base + TRNG_STATUS_OFFSET) & bit_mask;
}
...
static uint32_t trng_wait_startup(void __iomem *ce_base)
{
	...
	} while ((fsm_state == FSM_STATE_RESET) ||
...
void trng_wait_ready(void __iomem *ce_base, uint32_t vce_id)
{
	...
	do {
		fsm_state = readl((ce_base + CE0_TRNG_STATUS_OFFSET) +
						(CE_JUMP * vce_id)) & 0x1;
		i++;
...
uint32_t trng_init(void __iomem *ce_base)
{
	...
	if (RNG_OFF_TIMER_VAL < 0) {
		...
		value = reg_value(1, value, TRNG_FORCERUN_SHIFT,
							TRNG_FORCERUN_MASK);
		writel(value, ce_ctrl_base);
	} else {
		writel(RNG_OFF_TIMER_VAL, ce_base +
				REG_CE_TRNG_SWOFFTMRVAL_OFFSET);
	}
	/*REG_CE_TRNG_CLKDIV*/
	writel(RNG_CLKDIV, ce_base + REG_CE_TRNG_CLKDIV_OFFSET);
	/*REG_CE_TRNG_INITWAITVAL*/
	writel(RNG_INIT_WAIT_VAL, ce_base +
			REG_CE_TRNG_INITWAITVAL_OFFSET);

	/*Enable NDRNG*/
	value = readl(ce_ctrl_base);
	value = reg_value(RNG_NB_128BIT_BLOCKS, value,
			TRNG_NB128BITBLOCKS_SHIFT, TRNG_NB128BITBLOCKS_MASK);
	...
	/*REG_CE_TRNG_KEY0REG 0x8110 REG_CE_TRNG_KEY1RE 0x8114*/
	/*GREG_CE_TRNG_KEY2REG REG_CE_TRNG_KEY3REG */
	...
	/*Enable interrupts for health tests*/
	/*(repetition and adaptive proportion tests) & AIS31 test failures */
	//ba431_enable_health_test_irq();
	value = readl(ce_ctrl_base);
	value = reg_value(1, value, TRNG_ENABLE_SHIFT,
						TRNG_ENABLE_MASK);
	value = reg_value(0, value, TRNG_LFSREN_SHIFT,
						TRNG_LFSREN_MASK);
	value = reg_value(0, value, TRNG_TESTEN_SHIFT,
						TRNG_TESTEN_MASK);
	value = reg_value(0, value, TRNG_CONDBYPASS_SHIFT,
						TRNG_CONDBYPASS_MASK);
	value = reg_value(0, value, TRNG_INTENREP_SHIFT,
						TRNG_INTENREP_MASK);
	value = reg_value(1, value, TRNG_INTENPROP_SHIFT,
						TRNG_INTENPROP_MASK);
	value = reg_value(1, value, TRNG_INTENFULL_SHIFT,
						TRNG_INTENFULL_MASK);
	value = reg_value(1, value, TRNG_INTENPRE_SHIFT,
						TRNG_INTENPRE_MASK);
	value = reg_value(1, value, TRNG_INTENALM_SHIFT,
						TRNG_INTENALM_MASK);
	value = reg_value(0, value, TRNG_FORCERUN_SHIFT,
						TRNG_FORCERUN_MASK);
	value = reg_value(0, value, TRNG_HEALTHTESTBYPASS_SHIFT,
						TRNG_HEALTHTESTBYPASS_MASK);
	value = reg_value(0, value, TRNG_AIS31BYPASS_SHIFT,
						TRNG_AIS31BYPASS_MASK);
	value = reg_value(0, value, TRNG_HEALTHTESTSEL_SHIFT,
						TRNG_HEALTHTESTSEL_MASK);
	value = reg_value(0, value, TRNG_AIS31TESTSEL_SHIFT,
						TRNG_AIS31TESTSEL_MASK);
	value = reg_value(0x4, value, TRNG_NB128BITBLOCKS_SHIFT,
						TRNG_NB128BITBLOCKS_MASK);
	value = reg_value(1, value, TRNG_FIFOWRITESTARTUP_SHIFT,
						TRNG_FIFOWRITESTARTUP_MASK);
	writel(value, ce_ctrl_base);
	...
	if ((status & 0x80) != 0x0) {
		...
		for (i = 0; i < size_time; i++) {
			for (j = 0; j < CE_TRNG_FIFO_SIZE_VALUE; ) {
				rng_value = readl(baddr+j);
				pr_info("trng_get_rand_by_fifo rng_value=%x.\n",
						rng_value);
				memcpy(dst+i*CE_TRNG_FIFO_SIZE_VALUE+j,
						(void *)(&rng_value), 4);
				j = j+4;
			}
		...
		if (size_left > 0) {
			for (j = 0; j < size_left; ) {
				...
				pr_info("trng_get_rand_by_fifo rng_value=%x.\n",
						rng_value);
				memcpy(dst+size_time*CE_TRNG_FIFO_SIZE_VALUE+j,
						(void *)(&rng_value), 4);
				j = j+4;
			}
		}
