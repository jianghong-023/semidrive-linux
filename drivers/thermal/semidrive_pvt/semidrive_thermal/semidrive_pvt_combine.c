...
static const char * const pvr_combine_types[] = {
	[COMBINE_MAX_TEMP]	= "max",
	[COMBINE_AVG_TMP]	= "avg",
	[COMBINE_MIN_TMP]	= "min",
};

static enum combine_pvt_sensor_temp_type get_pvt_sensor_combine_type(const char *t)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(pvr_combine_types); i++)
		if (!strcasecmp(t, pvr_combine_types[i])) {
			return i;
		}
	return 0;
}

static int semidrive_combine_get_temp(void *data, int *temperature)
{
	struct semidrive_pvt_sensor *sensor = data;
	struct semidrive_pvt_controller *controller = sensor->combine->controller;
	...
	pvtprintk("semidrive_combine_get_temp %d millicelsius\n", (*temperature));
	return 0;
}

static struct semidrive_pvt_controller *
...
{
	struct semidrive_pvt_controller *controller = NULL;
	...
	for_each_child_of_node(np, combine_np) {
		...
		sprintf(combine_name, "semidrive_pvt_combine_%d", i);
...
EXPORT_SYMBOL(semidrive_pvt_controller_register);

void semidrive_pvt_controller_unregister(struct semidrive_pvt_controller *controller)
{
	struct semidrive_pvt_sensor *sensor;
...
EXPORT_SYMBOL(semidrive_pvt_controller_unregister);

static int semidrive_combine_parse(struct semidrive_pvt_sensor *sensor)
{
	...
	if (IS_ERR_OR_NULL(combine)) {
		pr_err("pvt combine: not enough memory for combine data\n");
		return -ENOMEM;
	}
	np = sensor->pdev->dev.of_node;
	combine->controller = combine_find_controller(np);
	if (!combine->controller) {
		pr_err("pvt sensor find no controller\n");
		goto parse_fail;
	}
	/* getting the pvt combine sensor have sensor num*/
	...
	/* getting the pvt combine sensor how to calcular all the sensor temp */
	if (of_property_read_string(np, "combine_sensor_temp_type", &type)) {
		pr_err("%s: get pvt combine type failed\n", __func__);
		return -EBUSY;
	} else {
		combine->type = get_pvt_sensor_combine_type(type);
	}

	/* getting the pvt combine sensor where focus on */
	if (of_property_read_string(np, "combine_sensor_type", &combine->combine_ths_type)) {
		pr_err("%s: get pvt sensor_num failed\n", __func__);
		return -EBUSY;
	}
	/* getting the pvt combine sensor include all sensor id */
	for (i = 0; i < combine->combine_ths_count; i++) {
		if (of_property_read_u32_index(np, "combine_sensor_id",
					i, &(combine->combine_ths_id[i]))) {
			pr_err("node pvt combine chn get failed!\n");
			goto parse_fail;
		}
	}
...
struct thermal_zone_of_device_ops combine_ops = {
	.get_temp          = semidrive_combine_get_temp,
};

static int semidrive_combine_probe(struct platform_device *pdev)
{
	int err = 0, id = 0;
	struct semidrive_pvt_sensor *sensor;
	struct semidrive_pvt_data *ths_data;
	if (!pdev->dev.of_node) {
		pr_err("can 't get the pvt combine ths node!\n");
		return -EBUSY;
	}
	sensor = kzalloc(sizeof(*sensor), GFP_KERNEL);
	if (IS_ERR_OR_NULL(sensor)) {
		pr_err("pvt combine: not enough memory for sensor data\n");
		return -EBUSY;
	}
	sensor->pdev = pdev;
	err = semidrive_combine_parse(sensor);
	if (err)
		goto fail;
	sscanf(pdev->name, "pvt_combine_%d", &id);
	sensor->sensor_id = id;
	ths_data = (struct semidrive_pvt_data *)sensor->combine->controller->data;
	...
	if (IS_ERR(sensor->tz)) {
		pr_err("semidrive pvt sensor register err!\n");
		goto fail1;
	}
...
static int semidrive_combine_remove(struct platform_device *pdev)
{
	struct semidrive_pvt_sensor *sensor;
...
#ifdef CONFIG_PM
static int semidrive_combine_suspend(struct device *dev)
{
	struct semidrive_pvt_sensor *sensor = dev_get_drvdata(dev);
	struct semidrive_pvt_controller *controller = sensor->combine->controller;
	pvtprintk("enter: semidrive_pvt_suspend.\n");
...
static int semidrive_combine_resume(struct device *dev)
{
	struct semidrive_pvt_sensor *sensor = dev_get_drvdata(dev);
	struct semidrive_pvt_controller *controller = sensor->combine->controller;
	int max_combine;
	pvtprintk("enter: semidrive_pvt_resume.\n");
...
static const struct dev_pm_ops semidrive_combine_pm_ops = {
	.suspend        = semidrive_combine_suspend,
	.resume         = semidrive_combine_resume,
};
#endif

static const struct of_device_id of_semidrive_combine_ths_match[] = {
	{ .compatible = "semidrive,pvt_combine" },
	{ /* end */ }
};

static struct platform_driver semidrive_combine_driver = {
	.probe  = semidrive_combine_probe,
	.remove = semidrive_combine_remove,
	.driver = {
		.name   = SEMIDRIVE_PVT_COMBINE_NAME,
		.owner  = THIS_MODULE,
	.of_match_table = of_semidrive_combine_ths_match,
#ifdef CONFIG_PM
		.pm	= &semidrive_combine_pm_ops,
#endif
	},
};

static int __init semidrive_pvt_combine_init(void)
{
	return platform_driver_register(&semidrive_combine_driver);
}

static void __exit semidrive_pvt_combine_exit(void)
{
	platform_driver_unregister(&semidrive_combine_driver);
	mutex_destroy(&controller_list_lock);
}

device_initcall_sync(semidrive_pvt_combine_init);
module_exit(semidrive_pvt_combine_exit);
