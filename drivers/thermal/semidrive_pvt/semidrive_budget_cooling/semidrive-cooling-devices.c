...
int semidrive_cpufreq_update_state(struct semidrive_budget_cooling_device *cooling_device, u32 cluster)
{
	...
	struct cpufreq_policy policy;
	struct semidrive_budget_cpufreq *cpufreq = cooling_device->cpufreq;
	if(NULL == cpufreq)
		return 0;
...
EXPORT_SYMBOL(semidrive_cpufreq_update_state);
...
EXPORT_SYMBOL(semidrive_cpufreq_get_roomage);
				...
				u32 freq_floor, u32 freq_roof, u32 cluster)
{
	...
	spin_unlock_irqrestore(&cpufreq->lock, flags);
	semidrive_cpufreq_update_state(cooling_device, cluster);
	return 0;
}
EXPORT_SYMBOL(semidrive_cpufreq_set_roomage);

static int cpufreq_thermal_notifier(struct notifier_block *nb,
					unsigned long event, void *data)
{
	struct semidrive_budget_cpufreq *cpufreq = container_of(nb ,struct semidrive_budget_cpufreq, notifer);
	struct semidrive_budget_cooling_device *bcd = cpufreq->bcd;
 ...
 * 
 */
static struct semidrive_cpufreq_cooling_table *
semidrive_cpufreq_cooling_parse(struct device_node *np, u32 tbl_num, u32 cluster_num)
{
...
semidrive_cpufreq_cooling_register(struct semidrive_budget_cooling_device *bcd)
{
	...
	tbl = semidrive_cpufreq_cooling_parse(np, bcd->state_num, bcd->cluster_num);
	if(!tbl){
		kfree(cpufreq);
		goto fail;
	}
...
EXPORT_SYMBOL(semidrive_cpufreq_cooling_register);
