...
static inline u32 kunlun_read(struct kunlun_gpio *gpio, unsigned int offset)
{
	...
	dev_err(gpio->dev, "kunlun_gpio read offset: [0x%8x], val[0x%8x]\n",
		offset, val);
	return val;
}
			       ...
			       u32 val)
{
	...
	dev_err(gpio->dev, "kunlun_gpio write offset: [0x%8x], val[0x%8x]\n",
		offset, val);
...
static int kunlun_gpio_to_irq(struct gpio_chip *gc, unsigned offset)
{
	...
	dev_err(gpio->dev, "%s: offset: [%d], irq[%d]\n",
		__func__, offset, irq);
...
static u32 kunlun_do_irq(struct kunlun_gpio_port *port)
{
	...
	dev_err(gpio->dev, "kunlun_gpio do_irq Enter\n");

	reg_idx = port->idx;

	dev_err(gpio->dev, "%s: reg_idx[%d], port->idx[%d]\n",
	...
	hwirq = bit_idx;
	dev_err(gpio->dev, "%s:  bit_idx[%d], hwirq[%d]\n",
	...
	/* inverse level for both edge trigger */
	kunlun_write(gpio,
		GPIO_TOGGLE_PIN_X(reg_idx * 32 + bit_idx),
		BIT(GPIO_CTRL_INT_POL_BIT));
	dev_err(gpio->dev, "do_irq: interrupt polarity toggled ! bit[%lu]\n",
		BIT(GPIO_CTRL_INT_POL_BIT));
	...
	while (irq_status) {
		...
		dev_err(gpio->dev, "irq_bit [%d], gpio_irq [%d]\n",
			irq_bit, gpio_irq);
...
static void kunlun_irq_handler(struct irq_desc *desc)
{
	...
	dev_err(gpio->dev, "kunlun_gpio %s Enter\n", __func__);
	kunlun_do_irq(port);
...
static void kunlun_irq_enable(struct irq_data *d)
{
	...
	/* enable debounce */
	kunlun_write(gpio,
		GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
		BIT(GPIO_CTRL_INT_DEB_EN_BIT));

	/* clock source select 32k(1) */
	kunlun_write(gpio,
		GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
		BIT(GPIO_CTRL_INT_CLK_SEL_BIT));

	kunlun_write(gpio,
		GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
		BIT(GPIO_CTRL_INT_LEV_SYNC_BIT));
	...
	kunlun_write(gpio,
		GPIO_CLEAR_PIN_X((reg_idx * 32 + bit_idx)),
		BIT(GPIO_CTRL_DIR_BIT));

	/* INT EN */
	kunlun_write(gpio,  GPIO_INT_EN_PORT_X(reg_idx), val);

	dev_err(gpio->dev, "%s: read again GPIO_CTRL\n", __func__);
...
static int kunlun_irq_set_type(struct irq_data *d, u32 type)
{
	...
	dev_err(gpio->dev, "%s: reg_idx[%d], hwirq[%lu], bit_idx[%u]\n",
		__func__, reg_idx, d->hwirq, bit_idx);

	switch (type) {
	case IRQ_TYPE_EDGE_BOTH:
		dev_err(gpio->dev, "%s IRQ_TYPE_EDGE_BOTH\n", __func__);
		kunlun_write(gpio, GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_TYPE_BIT) |
			BIT(GPIO_CTRL_INT_BOTH_EDGE_BIT));
		break;
	case IRQ_TYPE_EDGE_RISING:
		dev_err(gpio->dev, "%s IRQ_TYPE_EDGE_RISING\n", __func__);
		kunlun_write(gpio, GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_TYPE_BIT));
		kunlun_write(gpio, GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_POL_BIT));
	...
	case IRQ_TYPE_EDGE_FALLING:
		dev_err(gpio->dev, "%s IRQ_TYPE_EDGE_FALLING\n", __func__);
		kunlun_write(gpio, GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_TYPE_BIT));
	...
	case IRQ_TYPE_LEVEL_HIGH:
		dev_err(gpio->dev, "%s IRQ_TYPE_LEVEL_HIGH\n", __func__);
		kunlun_write(gpio, GPIO_CLEAR_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_TYPE_BIT));
		kunlun_write(gpio, GPIO_SET_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_POL_BIT));
	...
	case IRQ_TYPE_LEVEL_LOW:
		dev_err(gpio->dev, "%s IRQ_TYPE_LEVEL_LOW\n", __func__);
		kunlun_write(gpio, GPIO_CLEAR_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_TYPE_BIT));
		kunlun_write(gpio, GPIO_CLEAR_PIN_X((reg_idx * 32 + bit_idx)),
			BIT(GPIO_CTRL_INT_POL_BIT));
				   ...
				   unsigned offset, unsigned debounce)
{
	...
	dev_err(gpio->dev, "%s: offset [0x%8x], reg_idx [%u]\n",
				 ...
				 unsigned long config)
{
	...
	dev_err(gpio->dev, "%s: offset[%d], config[%lu]\n",
				 ...
				 struct kunlun_port_property *pp)
{
	...
	dev_err(gpio->dev, "kunlun_gpio configure_irqs Enter\n");
	...
	if (!pp->irq_shared) {
		dev_err(gpio->dev, "kunlun_gpio irq Not shared...\n");
		 ...
		 */
		dev_err(gpio->dev, "kunlun_gpio irq shared...\n");
...
static void kunlun_irq_teardown(struct kunlun_gpio *gpio)
{
	...
	dev_err(gpio->dev, "kunlun_gpio %s\n", __func__);

	for (idx = 0; idx < gpio->nr_ports; idx++) {
		dev_err(gpio->dev, "port idx[%d] removed!\n", idx);
		port = &gpio->ports[idx];
		gc = &port->gc;
		ngpio = gc->ngpio;
			       ...
			       unsigned int offs)
{
	...
	dev_err(gpio->dev, "kunlun_gpio %s Enter, offs [%d]\n",
		__func__, offs);
	...
	dev_err(gpio->dev, "kunlun_gpio port idx [%d], should < 5.\n", pp->idx);
	...
	dev_err(gpio->dev, "kunlun_gpio bgpio_init for port%d\n", port->idx);
...
kunlun_gpio_get_pdata(struct device *dev, struct kunlun_gpio *gpio)
{
	...
	dev_err(dev, "kunlun_gpio %s Enter\n", __func__);
	nports = device_get_child_node_count(dev);
	if (nports == 0)
		return ERR_PTR(-ENODEV);

	dev_err(dev, "kunlun_gpio [%d] ports found\n", nports);
	...
	device_for_each_child_node(dev, fwnode)  {
		...
		dev_err(dev, "kunlun_gpio: nr-gpios [%d]\n", pp->ngpio);
		...
		dev_err(dev, "i[%d], irq[%u], port_idx[%u], hwirq_start[%u]\n",
...
static int kunlun_gpio_probe(struct platform_device *pdev)
{
	...
	gpio->flags = 0;
	if (dev->of_node) {
		dev_err(dev, "kunlun_gpio of_node found !\n");
