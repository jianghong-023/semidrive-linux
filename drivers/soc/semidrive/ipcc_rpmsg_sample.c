...
static void build_msg_header(struct rpmsg_sample_device *ctx,
			     int command,
			     struct rpmsg_sample_header_msg *header)
{
	header->tag = RPMSG_TEST_SENITY_TAG;
	header->param1 = 0x11111111;
	header->param2 = 0x22222222;
...
/* from AP to remote */
static int rpmsg_sample_send_async(struct rpmsg_sample_device *sampdev, int command,
		   struct rpmsg_sample_param *param)
{
	struct rpmsg_device *rpmsg_device = sampdev->rpmsg_device;
	struct rpmsg_sample_cmd_msg msg;
...
int rpmsg_sample_open(struct rpmsg_sample_device *sampdev, struct rpmsg_sample_param *param)
{
	return rpmsg_sample_send_sync(sampdev, RPMSG_TEST_CMD_OPEN, param);
}

int rpmsg_sample_close(struct rpmsg_sample_device *sampdev, struct rpmsg_sample_param *param)
{
	return rpmsg_sample_send_sync(sampdev, RPMSG_TEST_CMD_CLOSE, param);
};

int rpmsg_sample_setdata_async(struct rpmsg_sample_device *sampdev, struct rpmsg_sample_param *param)
{
