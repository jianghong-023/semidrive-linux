...
#define RPCALLS_VERSIONS "1"
#define MAX_RING_ORDER XENBUS_MAX_RING_GRANT_ORDER

struct rpcalls_back_global {
...
struct sock_mapping {
	...
	struct rpcalls_ioworker ioworker;
};
...
static irqreturn_t rpcalls_back_conn_event(int irq, void *sock_map);
static int rpcalls_back_release_active(struct xenbus_device *dev,
				       struct rpcalls_fedata *fedata,
				       struct sock_mapping *map);

static void rpcalls_conn_back_read(void *opaque)
{
	...
	atomic_set(&map->read, 0);
	ret = sock_recvmsg(map->sock, &msg, MSG_DONTWAIT);
	WARN_ON(ret > wanted);
...
static void rpcalls_conn_back_write(struct sock_mapping *map)
{
	struct pvcalls_data_intf *intf = map->ring;
	struct pvcalls_data *data = &map->data;
	struct msghdr msg;
...
static void rpcalls_back_ioworker(struct work_struct *work)
{
	struct rpcalls_ioworker *ioworker = container_of(work,
		struct rpcalls_ioworker, register_work);
	...
	while (atomic_read(&map->io) > 0) {
		...
		if (atomic_read(&map->read) > 0)
			rpcalls_conn_back_read(map);
		if (atomic_read(&map->write) > 0)
			rpcalls_conn_back_write(map);

		atomic_dec(&map->io);
	}
}

static int rpcalls_back_socket(struct xenbus_device *dev,
		struct xen_pvcalls_request *req)
{
	...
	if (req->u.socket.domain != AF_RPMSG ||
...
static void rpcalls_sk_state_change(struct sock *sock)
{
...
static void rpcalls_sk_data_ready(struct sock *sock)
{
	struct sock_mapping *map = sock->sk_user_data;
	struct rpcalls_ioworker *iow;
...
static struct sock_mapping *rpcalls_new_active_socket(
		struct rpcalls_fedata *fedata,
		uint64_t id,
		grant_ref_t ref,
		uint32_t evtchn,
		struct socket *sock)
{
	...
	map->ioworker.wq = alloc_workqueue("rpcalls_io", WQ_UNBOUND, 1);
	if (!map->ioworker.wq)
		goto out;
	atomic_set(&map->io, 1);
	INIT_WORK(&map->ioworker.register_work,	rpcalls_back_ioworker);
	...
	map->sock->sk->sk_data_ready = rpcalls_sk_data_ready;
	map->sock->sk->sk_state_change = rpcalls_sk_state_change;
...
out:
	down(&fedata->socket_lock);
	list_del(&map->list);
	rpcalls_back_release_active(fedata->dev, fedata, map);
	up(&fedata->socket_lock);
	return NULL;
}

static int rpcalls_back_connect(struct xenbus_device *dev,
				struct xen_pvcalls_request *req)
{
	struct rpcalls_fedata *fedata;
	int ret = -EINVAL;
	...
	if (req->u.connect.len < sizeof(sa->sa_family) ||
	    ...
	    sa->sa_family != AF_RPMSG)
		goto out;

	ret = sock_create(sa->sa_family, SOCK_SEQPACKET, 0, &sock);
	if (ret < 0)
		goto out;
	ret = kernel_connect(sock, sa, req->u.connect.len, 0);
	if (ret < 0) {
		sock_release(sock);
		goto out;
	}

	map = rpcalls_new_active_socket(fedata,
					req->u.connect.id,
					req->u.connect.ref,
...
static int rpcalls_back_release_passive(struct xenbus_device *dev,
					struct rpcalls_fedata *fedata,
...
static int rpcalls_back_release(struct xenbus_device *dev,
				struct xen_pvcalls_request *req)
{
	struct rpcalls_fedata *fedata;
	...
	list_for_each_entry_safe(map, n, &fedata->socket_mappings, list) {
		if (map->id == req->u.release.id) {
			list_del(&map->list);
			up(&fedata->socket_lock);
			ret = rpcalls_back_release_active(dev, fedata, map);
			goto out;
		}
	}
	...
	if (mappass != NULL) {
		...
		up(&fedata->socket_lock);
		ret = rpcalls_back_release_passive(dev, fedata, mappass);
	} else
		up(&fedata->socket_lock);
...
static void __rpcalls_back_accept(struct work_struct *work)
{
	...
	/*
	 * __rpcalls_back_accept can race against rpcalls_back_accept.
	...
	sock = NULL;
	ret = kernel_accept(mappass->sock, &sock, O_NONBLOCK);
	if (ret == -EAGAIN) {
		goto out_error;
	}
...
static void rpcalls_pass_sk_data_ready(struct sock *sock)
{
...
static int rpcalls_back_bind(struct xenbus_device *dev,
			     struct xen_pvcalls_request *req)
{
	struct rpcalls_fedata *fedata;
	int ret;
	...
	INIT_WORK(&map->register_work, __rpcalls_back_accept);
	...
	ret = sock_create(AF_RPMSG, SOCK_SEQPACKET, 0, &map->sock);
	if (ret < 0)
		goto out;

	ret = kernel_bind(map->sock, (struct sockaddr *)&req->u.bind.addr,
			req->u.bind.len);
	if (ret < 0)
		goto out;
	...
	map->saved_data_ready = map->sock->sk->sk_data_ready;
	map->sock->sk->sk_user_data = map;
	map->sock->sk->sk_data_ready = rpcalls_pass_sk_data_ready;
	write_unlock_bh(&map->sock->sk->sk_callback_lock);
...
static int rpcalls_back_listen(struct xenbus_device *dev,
			       struct xen_pvcalls_request *req)
{
	return -ENOTSUPP;
}

static int rpcalls_back_accept(struct xenbus_device *dev,
			       struct xen_pvcalls_request *req)
{
	return -ENOTSUPP;
}

static int rpcalls_back_poll(struct xenbus_device *dev,
			     struct xen_pvcalls_request *req)
{
	struct rpcalls_fedata *fedata;
	struct sockpass_mapping *mappass;
	...
	data = !skb_queue_empty(&mappass->sock->sk->sk_receive_queue);
	if (data) {
		mappass->reqcopy.cmd = 0;
		ret = 0;
		goto out;
...
static int rpcalls_back_handle_cmd(struct xenbus_device *dev,
				   struct xen_pvcalls_request *req)
{
	int ret = 0;
	...
	switch (req->cmd) {
	case PVCALLS_SOCKET:
		ret = rpcalls_back_socket(dev, req);
		break;
	case PVCALLS_CONNECT:
		ret = rpcalls_back_connect(dev, req);
		break;
	case PVCALLS_RELEASE:
		ret = rpcalls_back_release(dev, req);
		break;
	case PVCALLS_BIND:
		ret = rpcalls_back_bind(dev, req);
		break;
	case PVCALLS_LISTEN:
		ret = rpcalls_back_listen(dev, req);
		break;
	case PVCALLS_ACCEPT:
		ret = rpcalls_back_accept(dev, req);
		break;
	case PVCALLS_POLL:
		ret = rpcalls_back_poll(dev, req);
		break;
	default:
	{
		struct rpcalls_fedata *fedata;
...
static void rpcalls_back_work(struct rpcalls_fedata *fedata)
{
	...
	while (more) {
		while (RING_HAS_UNCONSUMED_REQUESTS(&fedata->ring)) {
			...
			if (!rpcalls_back_handle_cmd(dev, &req)) {
...
static irqreturn_t rpcalls_back_event(int irq, void *dev_id)
{
	struct xenbus_device *dev = dev_id;
	struct rpcalls_fedata *fedata = NULL;
...
static int backend_connect(struct xenbus_device *dev)
{
	...
	fedata = kzalloc(sizeof(struct rpcalls_fedata), GFP_KERNEL);
	if (!fedata)
		return -ENOMEM;
	...
	err = request_threaded_irq(fedata->irq, NULL, rpcalls_back_event,
	...
	down(&rpcalls_back_global.frontends_lock);
	list_add_tail(&fedata->list, &rpcalls_back_global.frontends);
	up(&rpcalls_back_global.frontends_lock);
...
static int backend_disconnect(struct xenbus_device *dev)
{
	...
	list_for_each_entry_safe(map, n, &fedata->socket_mappings, list) {
		list_del(&map->list);
		rpcalls_back_release_active(dev, fedata, map);
	}
	...
	radix_tree_for_each_slot(slot, &fedata->socketpass_mappings, &iter, 0) {
		...
		if (radix_tree_exception(mappass)) {
			...
			radix_tree_delete(&fedata->socketpass_mappings,
					  mappass->id);
			rpcalls_back_release_passive(dev, fedata, mappass);
		}
	}
	up(&fedata->socket_lock);
...
static int rpcalls_back_probe(struct xenbus_device *dev,
...
static void rpcalls_back_changed(struct xenbus_device *dev,
				 enum xenbus_state frontend_state)
{
	dev_info(&dev->dev, "%s %p frontend(%s) this end(%s)\n",
			 __func__,
			 dev,
...
static int rpcalls_back_remove(struct xenbus_device *dev)
{
	return 0;
}

static int rpcalls_back_uevent(struct xenbus_device *xdev,
...
static struct xenbus_driver rpcalls_back_driver = {
	.ids = rpcalls_back_ids,
	.probe = rpcalls_back_probe,
	.remove = rpcalls_back_remove,
	.uevent = rpcalls_back_uevent,
	.otherend_changed = rpcalls_back_changed,
};

static int __init rpcalls_back_init(void)
{
	int ret;
	...
	ret = xenbus_register_backend(&rpcalls_back_driver);
	if (ret < 0)
		return ret;

	sema_init(&rpcalls_back_global.frontends_lock, 1);
	INIT_LIST_HEAD(&rpcalls_back_global.frontends);
	return 0;
}
module_init(rpcalls_back_init);

static void __exit rpcalls_back_fini(void)
{
	struct rpcalls_fedata *fedata, *nfedata;

	down(&rpcalls_back_global.frontends_lock);
	list_for_each_entry_safe(fedata, nfedata,
				 &rpcalls_back_global.frontends, list) {
		backend_disconnect(fedata->dev);
	}
	up(&rpcalls_back_global.frontends_lock);

	xenbus_unregister_driver(&rpcalls_back_driver);
}

module_exit(rpcalls_back_fini);

MODULE_ALIAS("rpmsg:rpcalls");
