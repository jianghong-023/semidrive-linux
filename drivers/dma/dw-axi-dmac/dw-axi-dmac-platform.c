...
static inline void
axi_dma_mux_iowrite32(struct axi_dma_chip *chip, u32 reg, u32 val)
{
...
static inline void
axi_chan_mux_iowrite32(struct axi_dma_chan *chan, u32 reg, u32 val)
{
...
static u32 bytes2block(size_t max_blocks, size_t bytes,
...
static struct axi_dma_desc *axi_desc_get(struct axi_dma_chan *chan)
{
	...
	desc->chan = chan;
	/* Set default to false, modify it at related function.  */
	desc->cyclic = false;
	return desc;
}
									  ...
									  struct axi_dma_desc *first)
{
	...
	case DMA_MEM_TO_DEV:
		...
		dev_vdbg(chan2dev(chan), "%s: direction(%d) hs(%d) L(0x%x) H(0x%x)\n", __func__, chan->direction, chan->dma_sconfig.slave_id, axi_chan_ioread32(chan, CH_CFG_L), axi_chan_ioread32(chan, CH_CFG_H));
		/* Set dma mux here MEM TO DEV -> WR */
		if (chan->chip->dw->hdata->dma_mux_enable == true)
		{
			/* Disable DMA MUX before setting*/
			axi_dma_mux_iowrite32(chan->chip, DMAC_MUX_EN, 0 << DMAC_MUX_EN_POS);
	...
	case DMA_DEV_TO_MEM:
		...
		if (chan->chip->dw->hdata->dma_mux_enable == true)
		{
			/* Disable DMA MUX before setting*/
			axi_dma_mux_iowrite32(chan->chip, DMAC_MUX_EN, 0 << DMAC_MUX_EN_POS);
			reg = (chan->dma_sconfig.slave_id << DMAC_MUX_RD_HDSK_SHIFT);
			axi_chan_mux_iowrite32(chan, DMAC_CHAN_MUX, reg);
			axi_dma_mux_iowrite32(chan->chip, DMAC_MUX_EN, 1 << DMAC_MUX_EN_POS);
			dev_vdbg(chan2dev(chan), "%s: RD_HDSK(0x%x) (0x%x) \n", __func__, reg, axi_chan_mux_ioread32(chan, DMAC_CHAN_MUX));
		}
		break;
	...
	//TODO:  if need block transfer interrupt, set here. dma_chan_prep_cyclic
	if (first->cyclic)
	{
...
static inline struct virt_dma_desc *vchan_submited_desc(struct virt_dma_chan *vc)
{
...
static void axi_chan_one_block_xfer_complete(struct axi_dma_chan *chan)
{
	...
	if (unlikely(!axi_chan_is_hw_enable(chan)))
	{
		dev_err(chan2dev(chan), "BUG: %s caught DWAXIDMAC_IRQ_DMA_BLK, but channel already disable!\n",
				axi_chan_name(chan));
		return;
	}
	spin_lock_irqsave(&chan->vc.lock, flags);
	...
	{
		//dev_vdbg(chan2dev(chan), "%s: cyclic interrupt!\n",axi_chan_name(chan));
...
static int dma_chan_config(struct dma_chan *dchan, struct dma_slave_config *sconfig)
{
	 ...
	 * 1 -> 0(1ITEM), 4 -> 1(4ITEMS), 8 -> 2(8ITEMS), 16 -> 3(16ITEMS). 32 -> 4
	 *
					   ...
					   unsigned long flags, void *context)
{
	...
	case DMA_MEM_TO_DEV:
		...
		slave_sg_todev_fill_desc:
			...
			desc->lli.block_ts_lo = cpu_to_le32(bytes2block(max_block_ts, len, mem_width, &dlen) - 1);
			...
			/* lli low */
			reg = (burst_items << CH_CTL_L_DST_MSIZE_POS | /* change to max burst */
			...
			len -= dlen;
			total_len += dlen;
			/* 	dev_dbg(chan2dev(chan), "%s: desc mem2dev: ctl_li: 0x%x ctl_lo: 0x%x src 0x%llx dst 0x%llx mem_addr 0x%llx sg_len 0x%x dlen: 0x%x len: %d total_len: %d block: %d flag: %d",
					axi_chan_name(chan), desc->lli.ctl_hi, desc->lli.ctl_lo, desc->lli.sar, desc->lli.dar, mem_addr, sg_len, dlen, len, total_len, desc->lli.block_ts_lo, flags);
		 */
			if (len)
...
/* Tested by I2S case */
static struct dma_async_tx_descriptor *dma_chan_prep_cyclic(
	...
	unsigned long flags)
{
	...
	for (offset = 0; offset < buf_len; offset += period_len)
	{
		...
		case DMA_MEM_TO_DEV:
			...
			/* calculate block size */
			desc->lli.block_ts_lo = cpu_to_le32(bytes2block(max_block_ts, period_len, reg_width, &dlen) - 1);
			...
			//lli low
			reg = (burst_items << CH_CTL_L_DST_MSIZE_POS | /* change to max burst */
				   burst_items << CH_CTL_L_SRC_MSIZE_POS | /* change to device */
		...
		total_len += dlen;
		/*dev_dbg(chan2dev(chan), "%s: desc mem2dev: ctl_li: 0x%x ctl_lo: 0x%x src 0x%llx dst 0x%llx buff 0x%x offset: 0x%x buf_len: %d total_len: %d block: %d flag: %d",		axi_chan_name(chan), desc->lli.ctl_hi, desc->lli.ctl_lo, desc->lli.sar, desc->lli.dar, buf_addr, offset, buf_len, total_len, desc->lli.block_ts_lo, flags);*/
	}
	...
	unsigned long flags)
{
	...
	return dma_chan_prep_dma_memcpy(dchan, buf_addr,
									buf_addr + 0x1000, 0x1000, flags); 
	struct scatterlist sgl[4], *sg;
	sg_init_table(sgl, 4);
...
static int dw_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < hdata->nr_channels; i++)
	{
		...
		{
			chan->chan_mux_regs = chip->mux_regs + i * CHAN_MUX_REG_LEN;
		}
	...
	dw->dma.directions = BIT(DMA_DEV_TO_MEM) | BIT(DMA_MEM_TO_DEV) | BIT(DMA_MEM_TO_MEM);
