				   ...
				   struct axi_dma_desc *desc_head)
{
	...
	dev_vdbg(dchan2dev(&chan->vc.chan), "llp(0x%llx) vd.tx.phys (0x%llx) total: %ud : %ud: %ud",
...
static void axi_chan_blocks_xfer_complete(struct axi_dma_chan *chan)
{
	...
	if (unlikely(axi_chan_is_hw_enable(chan))) {
		dev_err(chan2dev(chan),
			"BUG: %s caught DWAXIDMAC_IRQ_DMA_TRF, but channel not "
			"idle!\n",
			axi_chan_name(chan));
		axi_chan_disable(chan);
	}
		     ...
		     enum dma_transfer_direction direction, unsigned long flags)
{
	...
	for (offset = 0; offset < buf_len; offset += period_len) {
		...
		dev_vdbg(chan2dev(chan), "%s: desc cyclic: ctl_li: 0x%x"
		  " ctl_lo: 0x%x src 0x%llx dst 0x%llx buff 0x%x offset: 0x%x"
		  " buf_len: %d total_len: %d block: %d flag: %d",
