...
void unpack_data(sd_vi2c_t *sd_vi2c_p, int8_t status)
{
	...
	if (sd_vi2c_p->r_msg_len) {
		...
		if (sd_vi2c_p->htime_flag == sd_vi2c_p->dtime_flag) {
			...
			void *data, int len, void *priv, u32 src)
{
	...
	if (msg_type == I2C_HEAD) {
		sd_vi2c_arr[adap_num]->htime_flag = time_flag;
		...
		sd_vi2c_arr[adap_num]->dtime_flag = time_flag;
		data_info_t *r_data = (data_info_t *)payload;

		if (sd_vi2c_arr[adap_num]->r_msg
				&& (sd_vi2c_arr[adap_num]->htime_flag == sd_vi2c_arr[adap_num]->dtime_flag)) {
			memcpy(sd_vi2c_arr[adap_num]->r_msg_p, r_data->i2c_data,
				len - PAYLOAD_HEAD_LEN);
			sd_vi2c_arr[adap_num]->r_msg_p += len - PAYLOAD_HEAD_LEN;
		}

	} else if (msg_type == I2C_END) {
		sd_vi2c_arr[adap_num]->etime_flag = time_flag;
		...
		if (sd_vi2c_arr[adap_num]->r_msg
			...
			pr_err("%s, adapter[%u] err: r_msg=%p, len=%u, "
				"r_msg_len=%u, htime=%u, dtime=%u, etime=%u\n",
				__func__,
				adap_num,
				sd_vi2c_arr[adap_num]->r_msg,
				len,
				sd_vi2c_arr[adap_num]->r_msg_len,
				sd_vi2c_arr[adap_num]->htime_flag,
				sd_vi2c_arr[adap_num]->dtime_flag,
				sd_vi2c_arr[adap_num]->etime_flag);
		}
