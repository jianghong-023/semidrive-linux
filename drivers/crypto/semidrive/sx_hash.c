                                 ...
                                 block_t* data_out)
{
    ...
    flush_cache((((struct crypto_dev *)device)->sram_base+CE2_SRAM_PUB_HASH_OUT_ADDR_OFFSET), hash_len);
    ...
    if ((EXT_MEM == data_out->addr_type) && (NULL != data_out->addr)) {
        ...
        ce_hash_out.len = hash_len;

        ret = memcpy_blk_cache(device, data_out, &ce_hash_out, hash_len,false,true);
 ...
 */
static uint32_t sha_padding(hash_alg_t hash_alg, uint32_t remain_size, size_t total_len, block_t padding)
{
    ...
    uint32_t len_in_bits = total_len * 8;
    size_t i;
 ...
 * @return padsize length of the padding to be added
 */
static uint32_t md5_padding(hash_alg_t hash_alg, uint32_t remain_size, size_t total_len, block_t padding)
{
    ...
    block_t* data_in)
{
    ...
    key.len = 0;
    out.addr = (uint8_t*)CE2_SRAM_PUB_HASH_MULT_PART_UPDATE_IV_ADDR_OFFSET;
    out.addr_type = SRAM_PUB;
    out.len = digest_len;
    return sx_hash_internal(device, hash_alg, &iv, &key, OP_PART_HASH, data_in, &out);
