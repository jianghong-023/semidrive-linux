 ...
 * key (exponent) if the \p pub flag is set and then followed by:
 * - dp, dq, dinv if the \p crt flag is set
 * - lambda(N), d if \p crt flag is not set
 * @param pub Flag different than zero when public key should be also provided
 * @param crt Flag different than zero for CRT key generation is required
