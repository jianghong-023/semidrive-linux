...
int adjust_sg_array(struct csession *ses, int pagecount)
{
	...
	for (array_size = ses->array_size; array_size < pagecount;
	     array_size *= 2)
		;
	pr_info("reallocating from %d to %d pages",
                ...
                struct scatterlist **dst_sg)
{
	...
	if (src == dst) {	/* inplace operation */
		...
		if (unlikely(rc)) {
			pr_err("failed to get user pages for data IO");
			return rc;
		}
	...
	*dst_sg = NULL; /* default to ignore output */
	...
	if (likely(src)) {
		rc = __get_userbuf(src, src_len, 0, ses->readonly_pages,
					   ses->pages, ses->sg, task, mm);
		if (unlikely(rc)) {
			pr_err("failed to get user pages for data input");
			return rc;
		}
		*src_sg = ses->sg;
	}
	...
	if (likely(dst)) {
		...
		*dst_sg = ses->sg + ses->readonly_pages;
		...
		if (unlikely(rc)) {
			pr_err("failed to get user pages for data output");
