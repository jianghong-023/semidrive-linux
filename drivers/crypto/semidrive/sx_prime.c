	...
	bool *is_prime)
{
	...
	uint32_t status = pke_start_wait_status(device);
						  ...
						  bool *is_prime)
{
	...
	for (i = 0; i < iteration; i++) {
		...
		uint32_t status = rng_get_rand_less_ref(device, // 0 < rand < number-2
...
static uint32_t get_prime_modulo(void *device, uint8_t *number, uint32_t nr_of_bytes, uint32_t *mod)
{
	...
	uint32_t status = modular_reduce_odd(device,                    //tmp_block = number_block mod tmp_block
										 &number_block,
										 &tmp_block,
										 &tmp_block);
						   ...
						   uint32_t rounds)
{
	...
	*(candi_number->addr + candi_number->len - 1) |= 0x01;
	 ...
	 */
	uint32_t status = get_prime_modulo(device, candi_number->addr, candi_number->len, &mod);
