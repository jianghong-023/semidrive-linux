    ...
    block_t pub_key)
{
    ...
    //printf_binary("dsa_generate_public_key  p==", p.addr, p.len);
// printf_binary("dsa_generate_public_key  generator==", generator.addr, generator.len);
// printf_binary("dsa_generate_public_key  priv_key==", priv_key.addr, priv_key.len);

    // Fetch the results by PKE config 0x1 << BA414EP_MEMLOC_8 | 0x1 << BA414EP_MEMLOC_9
    ...
    block_t signature)
{
    ...
    error = hash_blk(hash_alg, vce_id, BLOCK_T_CONV(NULL, 0, 0), message, hash_block);
