						 ...
						 hash_alg_t hashType)
{
	...
	if (padding_type == ESEC_RSA_PADDING_OAEP) {
		status = rsa_pad_eme_oaep_encode(device, n->len, hashType, msg_padding, message, message->len);
		...
		block_temp = *message;
		status = rsa_pad_eme_pkcs_encode(device, n->len, msg_padding, &block_temp, message->len);
		...
		status = mem2CryptoRAM_rev(device, message, size, BA414EP_MEMLOC_5, true);
	...
	CryptoRAM2mem_rev(device, result, size, BA414EP_MEMLOC_4, true);
#endif
	ce_free(mem_n);
							 ...
							 hash_alg_t hashType)
{
	...
	if (padding_type != ESEC_RSA_PADDING_NONE) {
		...
		if (padding_type == ESEC_RSA_PADDING_OAEP) {
			error |= rsa_pad_eme_oaep_decode(device, p->len, hashType, msg_padding, &addr, (size_t *)msg_len);
		}
		else if (padding_type == ESEC_RSA_PADDING_EME_PKCS) {
			error |= rsa_pad_eme_pkcs_decode(p->len, msg_padding, &addr, (size_t *)msg_len);
		}
									  ...
									  uint32_t salt_length)
{
	...
	block_t data_out = BLOCK_T_CONV(hash, MAX_DIGESTSIZE, EXT_MEM);
	...
	error |= hash_blk(device, sha_type, &iv, message, &data_out);
	...
	// Location 12 -> hash
	error = mem2CryptoRAM_rev(device, &hash_block, size, BA414EP_MEMLOC_12, true);
										...
										uint32_t salt_length)
{
	...
	error = hash_blk(device, sha_type, &iv, message, &hash_block);
	...
	if (padding_type != ESEC_RSA_PADDING_PSS) {
		...
		error = mem2CryptoRAM_rev(device, signature, signature->len, BA414EP_MEMLOC_11, true);
		...
		error = mem2CryptoRAM_rev(device, signature, signature->len, BA414EP_MEMLOC_5, true);
	...
	if (padding_type == ESEC_RSA_PADDING_PSS) {
		...
		error = rsa_pad_emsa_pss_decode(device, n->len, sha_type, msg_padding, hash_block.addr,
										...
										uint32_t lambda)
{
	...
	// Location 2 -> P
	error = mem2CryptoRAM_rev(device, p, size, BA414EP_MEMLOC_2, true);
	...
	// Location 3 -> Q
	error = mem2CryptoRAM_rev(device, q, size, BA414EP_MEMLOC_3, true);
									...
									uint32_t size)
{
	...
	// Location 6 -> D
	error = mem2CryptoRAM_rev(device, d, size, BA414EP_MEMLOC_6, true);
	...
	CryptoRAM2mem_rev(device, inv, size, BA414EP_MEMLOC_12, true);
								...
								bool lambda)
{
	...
	do {
		...
		err = rsa_private_key_generation_blk(device, p, q, e, n, d,
											 nbr_of_bytes, 0);
	}
	...
	if (wrong_time > 500) {
		try_again = false;
		pr_err("rsa_key_generation_blk is err");
