...
uint32_t ecdsa_validate_domain(uint32_t vce_id, const sx_ecc_curve_t* curve)
{
    uint32_t size;

    size = ecc_curve_bytesize(curve);

    pke_set_command(vce_id, BA414EP_OPTYPE_ECDSA_PARAM_EVAL, size, BA414EP_LITTLEEND, curve->pk_flags);
    pke_load_curve(vce_id, curve->params, size, BA414EP_LITTLEEND, 1);

    return pke_start_wait_status(vce_id);
}

uint32_t ecdsa_configure_signature(uint32_t vce_id,
...
uint32_t ecdsa_attempt_signature(uint32_t vce_id,
                                 const sx_ecc_curve_t* curve,
                                 block_t signature)
{
    //void * sign_temp = NULL;
...
uint32_t ecdsa_generate_signature_digest(uint32_t vce_id,
        const sx_ecc_curve_t* curve,
        block_t formatted_digest,
        block_t key,
        block_t signature)
...
uint32_t ecdsa_verify_signature_digest(uint32_t vce_id,
                                       const sx_ecc_curve_t* curve,
                                       block_t formatted_digest,
                                       block_t key,
                                       block_t signature)
                                      ...
                                      block_t* digest_blk)
{
    ...
    if (digest_blk->len < hash_len || curve_order_bytesize < hash_len) { //if curve_order_bytesize < hash_len, please use more len function
        return CRYPTOLIB_INVALID_PARAM;
    }

    /* Call hash fct to get digest. */
    status = hash_blk(hash_fct, vce_id, BLOCK_T_CONV(NULL, 0, 0), message, *digest_blk);
...
uint32_t ecdsa_generate_signature(uint32_t vce_id,
...
uint32_t ecdsa_verify_signature(uint32_t vce_id,
                                const void* curve_info,
                                block_t message,
                                block_t key,
                                block_t signature,
