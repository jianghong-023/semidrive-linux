 ...
 */
static uint32_t eddsa_load_parameters(uint32_t vce_id, eddsa_curve_t curve)
{
    pke_load_curve(vce_id,
 ...
 * @return CRYPTOLIB_SUCCESS
 */
static uint32_t eddsa_hash_and_clamp(uint32_t vce_id,
                                     eddsa_curve_t curve,
                                     block_t sk,
                                     uint8_t* result,
                                     const size_t result_size)
{
    ...
    if (curve == SX_ED25519) {
        status = hash_blk(ALG_SHA512, vce_id, BLOCK_T_CONV(NULL, 0, 0), sk, BLOCK_T_CONV(result, result_size, EXT_MEM));
        ...
        status = hash_blk(ALG_SHA256, vce_id, BLOCK_T_CONV(NULL, 0, 0), sk, BLOCK_T_CONV(result, ED448_SIGN_SIZE, EXT_MEM));
    ...
    block_t pk)
{
    ...
    status = eddsa_hash_and_clamp(vce_id, curve, sk, buffer, 2 * key_size);
    ...
    point2CryptoRAM_rev(vce_id, BLOCK_T_CONV(buffer, key_size * 2, EXT_MEM),
                        key_size, BA414EP_MEMLOC_8);

    // Set command to disable byte-swap
    pke_set_command(vce_id, BA414EP_OPTYPE_EDDSA_POINT_MULT, key_size,
                    BA414EP_LITTLEEND, eddsa_hw_flags[curve]);

    // Load parameters
    eddsa_load_parameters(vce_id, curve);
    ...
    CryptoRAM2point_rev(vce_id, BLOCK_T_CONV(buffer, key_size * 2, EXT_MEM),
 ...
 */
uint32_t eddsa_generate_signature(uint32_t vce_id,
                                  eddsa_curve_t curve,
                                  block_t k,
                                  block_t r,
                                  block_t s,
                                  block_t sig)
{
    ...
    pke_set_command(vce_id, BA414EP_OPTYPE_EDDSA_SIGN_GEN, key_size,
                    BA414EP_LITTLEEND, eddsa_hw_flags[curve]);

    /* Prepare EdDSA parameters for signature generation */
    point2CryptoRAM_rev(vce_id, k, key_size, BA414EP_MEMLOC_6);
    point2CryptoRAM_rev(vce_id, r, key_size, BA414EP_MEMLOC_8);
    mem2CryptoRAM_rev(vce_id, s, key_size, BA414EP_MEMLOC_11, true);
    ...
    CryptoRAM2mem_rev(vce_id, sig, key_size, BA414EP_MEMLOC_10, true);
...
uint32_t eddsa_verify_signature(uint32_t vce_id,
                                eddsa_curve_t curve,
                                block_t k,
                                block_t pk_y,
                                block_t r_y,
                                block_t sig)
{
    ...
    // Set command to disable byte-swap
    pke_set_command(vce_id, BA414EP_OPTYPE_EDDSA_SIGN_VERIF,
                    key_size,
                    BA414EP_LITTLEEND,
    ...
    point2CryptoRAM_rev(vce_id, k, key_size, BA414EP_MEMLOC_6);
    mem2CryptoRAM_rev(vce_id, pk_y, key_size, BA414EP_MEMLOC_9, true);
    mem2CryptoRAM_rev(vce_id, sig, key_size, BA414EP_MEMLOC_10, true); // S
    mem2CryptoRAM_rev(vce_id, r_y, key_size, BA414EP_MEMLOC_11, true); // R
