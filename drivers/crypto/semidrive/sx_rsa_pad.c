				 ...
				 size_t maskLen)
{
	...
	while (maskLen >= hashLen) {
		...
		data_out =  BLOCK_T_CONV(mask, maskLen, EXT_MEM);
								 ...
								 size_t mLen)
{
	...
	block_t data_in = BLOCK_T_CONV(NULL, hLen, 0);
	...
	//Assemply of DB                //随机产生数字 random
	gen_rnd_bytes(device, EM + 1, hLen, 0);

	EM[0] = 0x00;
	memset((uint8_t *)(EM + 2 * hLen + 1), 0x00, k - mLen - 2 * hLen - 2); //PS初始化函数
	*((uint8_t *)(EM + k - mLen - 1)) = 0x01; //0x01     添加1
	memcpy(EM + k - mLen, message->addr, mLen);  //复制明文到EM上
	...
	MGF1(device, hashType, EM + 1 + hLen, k - hLen - 1, seedMask, hLen);
								 ...
								 size_t *mLen)
{
	...
	*mLen = 0;
	...
	iv = BLOCK_T_CONV(NULL, 0, 0);
	data_in = BLOCK_T_CONV(NULL, hLen, 0);
								 ...
								 size_t mLen)
{
	...
	//Assemply of DB
	gen_rnd_bytes(device, EM + 2, k - mLen - 2, 1); //PS (first written for alignment purpose)
	EM[0] = 0x00;
	EM[1] = 0x02;
