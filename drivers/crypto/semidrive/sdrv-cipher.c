...
static void semidrive_ablkcipher_done(void *data)
{
	return;
}
...
semidrive_ablkcipher_async_req_handle(struct crypto_async_request *async_req)
{
	...
	input_buff = kzalloc(rctx->cryptlen + CACHE_LINE, GFP_KERNEL);
	if (!input_buff) {
		return -ENOMEM;
	}
	...
	output_buff = kzalloc(rctx->cryptlen + CACHE_LINE, GFP_KERNEL);
	if (!output_buff) {
	...
	for (i = 0; i < rctx->src_nents; i++) {
		memcpy(input_buff + input_offset + cpy_offset,
	...
	ret = aes_blk(alg_fct, sdce, alg_mode,
                 ...
                 (block_t)BLOCK_T_CONV(iv, rctx->ivsize, EXT_MEM),
                 (block_t)BLOCK_T_CONV(NULL, 0, EXT_MEM),
                 (block_t)BLOCK_T_CONV(input_buff + input_offset,
				 rctx->cryptlen, EXT_MEM),
                 &BLOCK_T_CONV(output_buff + output_offset,
				 rctx->cryptlen, EXT_MEM));
	if (!ret) {
		cpy_offset = 0;
		for (i = 0; i < rctx->src_nents; i++) {
			memcpy(sg_virt(&rctx->dst_sg[i]),
				output_buff + output_offset + cpy_offset,
