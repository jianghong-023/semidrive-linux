...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_CTRL);
...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_CLK_CFG);
...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_MCHC);
...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_SSC);
...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_IMASK);
...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_TIMER);
...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_FIFO);
...
{
	u32 write_v = readl(info->regs + REG_AP_APB_ADC_ADC_DMA_CFG);
...
{
	unsigned long input_clk = clk_get_rate(info->clk);
...
static int sdrv_adc_probe(struct platform_device *pdev)
{
	...
	indio_dev->num_channels = ARRAY_SIZE(sdrv_adc_iio_channels);
	...
	ret = devm_request_irq(info->dev, irq, sdrv_adc_isr, 0, dev_name(&pdev->dev), info);
	if (ret < 0) {
