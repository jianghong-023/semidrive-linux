		...
		uint32_t num_grefs, grant_ref_t *grefs)
{
	...
	int i, ret=0;
	size_t size = num_grefs * PAGE_SIZE;
	struct page *psPage;
	uint32_t *vaddr = NULL;
...

	{
		...
		//PVRSRV_DATA *pvrsrvdata = PVRSRVGetPVRSRVData();
		//xen_pfn_t *frame_list;
		size_t size = num_pages * PAGE_SIZE;
		//struct device * pvrsrvdev = (struct device *)pvrsrvdata->psDeviceNodeList->psDevConfig->pvOSDevice;
		...
		if (!frame_list) {
			VMM_ERROR_PRINT("%s, Failed to allocate frames to decrease reservation\n", __FUNCTION__);
			kfree(frame_list);
			return -ENOMEM;
		}*/

		pr_err("in __xdrv_map_dev_heap, pvOSDevice = %p", pvrsrvdev);
		...
		/*vaddr = dma_alloc_wc(pvrsrvdev, size, &dev_addr, GFP_KERNEL | __GFP_NOWARN);
		if (!vaddr) {
			VMM_ERROR_PRINT("Failed to allocate DMA buffer with size %zu\n", size);
			kfree(frame_list);
			return -ENOMEM;
...
/*		ret = gnttab_alloc_pages(num_pages, pages);
		if (ret || !(*pages)) {
			VMM_ERROR_PRINT("in %s:%d\n", __FUNCTION__, __LINE__);
			goto fail;
		}
		vaddr=xen_page_to_vaddr(pages[0]);
		dev_addr=virt_to_phys(dev_addr);*/
		...
		VMM_DEBUG_PRINT("vaddr 0x%p, cpu_addr 0x%p!!!!!!\n", vaddr, xen_page_to_vaddr(pages[0]));
