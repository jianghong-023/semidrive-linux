...
int xengpu_do_request(struct gpuif_request *pReq, struct gpuif_response **rsp)
{
	...
	VMM_DEBUG_PRINT("%s:%d, xen gpu is initialized correctly, pReq->id = %d!", __FUNCTION__, __LINE__, pReq->id);
	...
	switch (call->rsp->operation)
	{
		...
		{
			ret = xdrv_map_dev_heap((struct xen_drvinfo *)&gXenGpuInfo,
				call->rsp->op.map_dev_heap.gref_directory,
				call->rsp->op.map_dev_heap.buffer_sz);
			if (!ret) {
				call->rsp->ui64FwPhysHeapAddr = gXenGpuInfo.heap_obj->addr;
			}
			VMM_DEBUG_PRINT("Get FW heap IPA 0x%x sz %llu, ret %d",
				call->rsp->ui64FwPhysHeapAddr,
				call->rsp->op.map_dev_heap.buffer_sz, ret);
			break;
		}
		case GPUIF_OP_DESTROY_DEVPHYSHEAPS:
			ret = xdrv_unmap_dev_heap((struct xen_drvinfo *)&gXenGpuInfo);
			break;
		default:
			break;
	}
...
{
    pr_debug("%s %p backend(%s) this end(%s)\n",
	     __func__,
	     dev,
