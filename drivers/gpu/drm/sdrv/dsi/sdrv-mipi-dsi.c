...
extern const struct dsi_core_ops dwc_mipi_dsi_host_ops;
...
static void dpi_video_params_init(struct dsi_context *ctx)
{
	...
	dpi_video->data_hs2lp = 50;
	dpi_video->data_lp2hs = 153;
	dpi_video->clk_hs2lp = 66;
	dpi_video->clk_lp2hs = 181;
...
static int mipi_dsih_dpi_video(struct dsi_context *ctx)
{
    ...
    dsi_ops->dpi_hsync_time(ctx, dpi_video->h_sync_pixels); //hsync
    dsi_ops->dpi_hbp_time(ctx, dpi_video->h_back_porch_pixels);//hbp
    dsi_ops->dpi_hline_time(ctx, dpi_video->h_total_pixels); //hsync+hbp_hact+hfp
...
static int sdrv_mipi_dsi_bridge_attach(struct sdrv_mipi_dsi *dsi)
{
	struct drm_encoder *encoder = &dsi->encoder;
	struct device *dev = dsi->dsi_host.dev;
