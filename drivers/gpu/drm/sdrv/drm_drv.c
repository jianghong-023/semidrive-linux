...
int drm_get_physic_memory_ioctl(struct drm_device *drm, void *data, struct drm_file *file)
{
	...
	if (NULL == temp) {
	    DRM_DEV_INFO(drm->dev,"[drm] get dma buf error \n");
	    return -1;
	}
...
int drm_export_dmabuf_from_phys_ioctl(struct drm_device *drm, void *data, struct drm_file *file)
{
	...
	struct dma_buf *buf = drm_mb_export(drm_buf->phys_addr, drm_buf->size, 2);
	if (IS_ERR(buf))
		return -EFAULT;
...
static const struct drm_ioctl_desc semidrive_ioctls[] = {
	...
	DRM_IOCTL_DEF_DRV(SEMIDRIVE_EXPORT_DMABUF, drm_export_dmabuf_from_phys_ioctl, 0),
