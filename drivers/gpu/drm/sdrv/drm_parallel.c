...
static int kunlun_get_display_info(struct device_node *np,  struct drm_display_info *disp_info, struct drm_display_mode *mode)
{
	...
	width_mm = of_property_read_u32(entry, "width-mm", 292);
	height_mm = of_property_read_u32(entry, "height-mm", 110);
	vrefresh = of_property_read_u32(entry, "vrefresh", 0);

	disp_info->width_mm = width_mm;
	disp_info->height_mm = height_mm;
