			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_prt(component->dev, "%s() event=0x%0x\n", __func__, event);
			     ...
			     struct snd_soc_dai *dai)
{
	...
	dev_prt(component->dev, "%s() rate=%u width=%u\n", __func__, rate, width);
...
static int tas6424_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	dev_prt(component->dev, "%s() fmt=0x%0x\n", __func__, fmt);
				    ...
				    int slots, int slot_width)
{
	...
	dev_prt(component->dev, "%s() tx_mask=%d rx_mask=%d\n", __func__,
		tx_mask, rx_mask);
...
static int tas6424_power_off(struct snd_soc_codec *codec)
{
	...
	int ret;
	regmap_write(tas6424->regmap, TAS6424_CH_STATE_CTRL, TAS6424_ALL_STATE_HIZ);
...
static int tas6424_power_on(struct snd_soc_codec *codec)
{
	...
	regmap_write(tas6424->regmap, TAS6424_CH_STATE_CTRL, chan_states);
