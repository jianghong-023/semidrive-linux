...
static int snd_x9_icl02_ak7738_controls_get(struct snd_kcontrol *kcontrol,
...
static int snd_x9_icl02_ak7738_controls_put(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	/*  Add control here.
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol); */
...
/*
 * Logic for a tas5404 as connected on a x9_icl02 amp
 */
static int x9_icl02_tas5404_init(struct snd_soc_pcm_runtime *rtd)
{
	int err;
	DEBUG_FUNC_PRT;
...
static int x9_icl02_hs_ak4556_init(struct snd_soc_pcm_runtime *rtd)
{
	int err;
	DEBUG_FUNC_PRT;
	return err;
...
MODULE_DEVICE_TABLE(of, x9_icl02_hs_dt_match);
