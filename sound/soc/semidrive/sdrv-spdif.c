...
#define CAL_SPDIF_SAMPLERATE_CODE(SAMPLE_RATE)  ((2 * SPDIF_CLK + 128 * SAMPLE_RATE) / (2 * 128 * SAMPLE_RATE) - 1)
		...
		struct snd_soc_dai *dai)
{
	...
	unsigned tsrate_code = CAL_SPDIF_SAMPLERATE_CODE(srate);
...
static irqreturn_t sdrv_spdif_irq_handler(int irq, void *dev_id)
{
	...
	if (spdif_status & BIT_SPDIF_INT_LOCK) {
		...
		switch (rx_rate) {
			case CAL_SPDIF_SAMPLERATE_CODE(16000):	afe->spdif_status = SDRV_SPDIF_STATUS_16_IN;
													break;
			case CAL_SPDIF_SAMPLERATE_CODE(48000):	afe->spdif_status = SDRV_SPDIF_STATUS_48_IN;
													break;
			default: afe->spdif_status = SDRV_SPDFI_STATUS_UNDEFINED;
		}
